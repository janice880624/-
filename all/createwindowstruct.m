...
window(394:1074,740:1250)=1;
sizeheight=windowsize(1);
sizewidth=windowsize(2);
%% Countdown font setting
% clock=imread('clock.jpg');
% clock=imresize(clock,[sizewidth*13/110 sizewidth*13/110]);
% clock=im2double(rgb2gray(clock));
% sizetime=size(clock);
% window(round(sizeheight/12):round(sizeheight/12)+sizetime(1)-1,round(sizewidth/2-sizetime(2)/2:sizewidth/2+sizetime(2)/2-1))=clock;
%% Number setting
